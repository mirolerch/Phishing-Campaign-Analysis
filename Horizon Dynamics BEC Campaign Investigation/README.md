# Phishing Campaign Analysis

A collection of phishing-investigation case studies completed as part of my transition from network and systems administration into cybersecurity / SOC work.

Each project is a self-contained investigation: emails under analysis, IOC tables, technical write-up, and (where applicable) a final campaign report.

---

## Projects

### [Horizon Dynamics BEC Campaign Investigation](#)

A two-stage Business Email Compromise (BEC) investigation against a fictional company, completed during a guided live CTF. Walks through a wide-net CEO-impersonation lure (Part 1), a targeted spearphishing follow-up with a macro-enabled attachment, encoded PowerShell, and LSB steganography (Part 2), and concludes with a one-page campaign analysis report (Part 3).

**Skills demonstrated:** Email triage, header analysis (SPF / DKIM / DMARC), IOC extraction, Base64 decoding via CyberChef, LSB steganography extraction via CyberChef, campaign-level reporting, risk rating.

**Outcome:** Instructor-reviewed, 10/10 on Parts 1 and 2, 9.5/10 on Part 3.

---

## About me

I'm transitioning from network and systems administration into SOC analyst work. This repo is part of that journey — every project captures a real piece of investigative practice plus my own reflections on what each one taught me.

See my other repo, [tshark-network-traffic-analysis](#), for network traffic-focused work.

— mirolerch, 2026
